...
def count_quantifiable_achievements(text: str) -> int:
    """
    Count achievement lines containing measurable information.

    Numbers, percentages, currency values, and common measurement
    expressions are treated as measurable signals.
    """
...
def analyze_resume_length(resume: dict) -> dict:
...
def analyze_section_lengths(resume: dict) -> dict:
...
def analyze_bullet_usage(resume: dict) -> dict:
    ...
    bullet_counts = {
        section: count_bullets(_text(resume.get(section)))
        for section in sections
    }
...
def analyze_achievements(resume: dict) -> dict:
    ...
    measurable = count_quantifiable_achievements(
        achievements
    )

    return {
        "total": len(achievement_lines),
        "quantifiable": measurable,
        "has_achievements": bool(achievements),
        "has_quantifiable": measurable > 0,
    }


def analyze_contact(resume: dict) -> dict:
    """Check basic contact information."""

    name_found = bool(_text(resume.get("name")))
    email_found = bool(_text(resume.get("email")))
    phone_found = bool(_text(resume.get("phone")))
...
def analyze_structure(resume: dict) -> dict:
    ...
    present_sections = [
        section
        for section in sections
        if _text(resume.get(section))
    ]

    missing_sections = [
        section
        for section in sections
        if not _text(resume.get(section))
    ]
...
def calculate_quality_score(
    ...
    if length["rating"] == "good":
        breakdown["length"] = 15
    elif length["rating"] in {"short", "long"}:
        breakdown["length"] = 8
    else:
        breakdown["length"] = 0
    ...
    breakdown["sections"] = round(
        section_ratio * QUALITY_WEIGHTS["sections"]
    )
    ...
    # Contact: 10 points
    contact_ratio = (
        contact["passed"] / contact["total"]
        if contact["total"]
        else 0
    )

    breakdown["contact"] = round(
        contact_ratio * QUALITY_WEIGHTS["contact"]
    )
...
def generate_quality_suggestions(
    ...
    if sections["summary"]["word_count"] > 100:
        suggestions.append(
...
def analyze_resume_quality(resume: dict) -> dict:
    ...
    length = analyze_resume_length(resume)
